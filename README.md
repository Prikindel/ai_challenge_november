...
- [lesson-20-dev-assistant](./lesson-20-dev-assistant/) — ассистент разработчика: RAG для документации проекта (project/docs/ и project/README.md), MCP для git (текущая ветка через get_current_branch), команда /help для вопросов о структуре проекта, API и схеме данных
- [lesson-21-code-review](./lesson-21-code-review/) — автоматизация ревью кода: система автоматического ревью кода с использованием RAG (документация и код), MCP для получения diff и файлов из PR, генерация структурированного ревью с проблемами, багами и предложениями по улучшению
- [lesson-22-support-assistant](./lesson-22-support-assistant/) — ассистент для поддержки пользователей: мини-сервис для ответов на вопросы пользователей с использованием RAG (FAQ и документация), MCP для интеграции с CRM (контекст о пользователе и тикете), ответы с учётом контекста тикета
- [lesson-23-team-assistant](./lesson-23-team-assistant/) — ассистент команды: интегрированный ассистент, объединяющий RAG (знание проекта), MCP (взаимодействие с Task Management), создание задач через API, ответы на вопросы о статусе проекта и рекомендации по приоритетам
- [lesson-24-real-task](./lesson-24-real-task/) — реальная задача: реализация реальной практической задачи с использованием всех накопленных знаний (RAG, MCP, LLM) — автоматизация деплоя, публикации в сторы, CI/CD, мониторинга и других задач
...
- [lesson-30-personalization](./lesson-30-personalization/) — персонализация: добавление персонализации агенту через конфиг (профиль пользователя, предпочтения, стиль работы, контекст), адаптация ответов под пользователя, создание личного агента
- [lesson-31-voice-agent](./lesson-31-voice-agent/) — голосовой агент: распознавание речи через Vosk (локально), отправка текста в локальную LLM, возврат текстового ответа (Speech → LLM → Text)
- [lesson-32-god-agent](./lesson-32-god-agent/) — God Agent: финальный персональный AI-помощник, объединяющий все наработки (RAG для базы знаний, модульные MCP серверы как плагины, голосовой ввод, персонализация, аналитика данных)
