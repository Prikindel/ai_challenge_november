...
- **Mistral 7B Instruct v0.3** — компактная и быстрая модель, оптимизированная под inference. Выдаёт лаконичные ответы, уверенно справляется с базовыми рассуждениями и кодом.
- **Falcon 7B Instruct** — экономичный 7B с открытой лицензией; ответы сдержанные и предсказуемые, хорошо подходит для проверок фактов и кратких резюме.
- **Zephyr 7B Beta (Hugging Face H4)** — тонко настроенная на человеческие предпочтения модель, даёт дружелюбные и структурированные ответы без дополнительных лицензий.
- **OpenChat 3.5-0106** — open-source chat-модель с упором на многошаговые рассуждения и диалоговый стиль, свободно доступна через HF Inference.
