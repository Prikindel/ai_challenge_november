...
/**
 * Конфигурация Task MCP
 */
data class TaskMCPConfig(
    val enabled: Boolean = true,
    val jarPath: String? = null
)
...
data class TaskConfig(
    val mcp: TaskMCPConfig = TaskMCPConfig()
)
...
data class AppConfig(
    ...
    val task: TaskConfig = TaskConfig()
)
...
object Config {
    ...
    fun load(): AppConfig {
        ...
        // Конфигурация Task
        val taskMap = serverConfigMap["task"] as? Map<String, Any> ?: emptyMap()
        val taskMCPMap = taskMap["mcp"] as? Map<String, Any> ?: emptyMap()
        val taskMCP = TaskMCPConfig(
            enabled = (taskMCPMap["enabled"] as? Boolean) ?: true,
            jarPath = taskMCPMap["jarPath"] as? String
        )
        val task = TaskConfig(mcp = taskMCP)
    ...
    /**
     * Находит корень проекта (lesson-23-team-assistant)
     */
    ...
    private fun findConfigDirectory(): String {
        ...
        // Ищем lesson-23-team-assistant вверх по дереву
        var searchDir = currentDir
        while (searchDir != null && searchDir.parentFile != null) {
            if (searchDir.name == "lesson-23-team-assistant") {
            ...
            val lessonDir = File(searchDir, "lesson-23-team-assistant")
            if (lessonDir.exists()) {
        ...
        throw IllegalStateException(
            ...
            "Please ensure config/server.yaml exists in lesson-23-team-assistant directory."
        )
    }
}
