...
fun main(args: Array<String>) {
    ...
    logger.info("Starting lesson-25-local-llm server...")
    ...
    embeddedServer(Netty, port = config.server.port, host = config.server.host) {
        ...
        routing {
            ...
            // Health check API
            get("/api/health") {
                call.respond(HttpStatusCode.OK, HealthResponse("ok", "lesson-25-local-llm-server"))
            }
...
/**
 * Находит корень урока (lesson-25-local-llm)
 */
private fun findLessonRoot(): File {
    ...
    // Ищем lesson-25-local-llm вверх по дереву
    ...
    while (searchDir != null) {
        ...
        val lessonDir = File(searchDir, "lesson-25-local-llm")
        if (lessonDir.exists()) {
            return lessonDir
        }
