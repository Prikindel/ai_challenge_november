...
class LocalLLMClient(
    ...
    suspend fun generate(
        ...
        return try {
            ...
            logger.info("Connection Type: $connectionType")
            logger.info("")
            // Логируем полный ответ (может быть длинным, но это важно для отладки)
            if (responseText.length > 5000) {
                logger.info("Response (first 5000 chars): ${responseText.take(5000)}...")
                logger.info("... (response truncated, total length: ${responseText.length} chars)")
            } else {
