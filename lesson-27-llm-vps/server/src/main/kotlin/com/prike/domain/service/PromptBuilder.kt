...
- Помогать разработчикам понимать, как работает проект"""
    ...
    private fun buildInstructions(): String {
        return """
            ...
            - Название документа и путь к документу указывай точно из контекста выше (из раздела "Доступные документы")
            - Примеры правильных цитат:
              * [Источник: API Документация](project/docs/api.md)
              * [Источник: Архитектура проекта](project/docs/architecture.md)
              * [Источник: MCP Guide](MCP_GUIDE.md)
            ...
            - Цитаты должны быть расположены сразу после соответствующих утверждений
            - Без цитат ответ считается неполным и неинформативным
            - ВАЖНО: Путь к документу должен точно совпадать с одним из путей в разделе "Доступные документы" выше
        """.trimIndent()
    }
