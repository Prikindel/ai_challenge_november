...
object Config {
    ...
    /**
     * Находит корень проекта (lesson-25-local-llm)
     */
    ...
    private fun findConfigDirectory(): String {
        ...
        // Ищем lesson-27-llm-vps вверх по дереву
        var searchDir = currentDir
        while (searchDir != null && searchDir.parentFile != null) {
            ...
            val lessonDir = File(searchDir, "lesson-27-llm-vps")
            if (lessonDir.exists()) {
            ...
            // Также проверяем lesson-25-local-llm для обратной совместимости
            if (searchDir.name == "lesson-25-local-llm") {
                configDir = File(searchDir, "config")
                if (configDir.exists() && File(configDir, "server.yaml").exists()) {
                    logger.info("Found config in lesson-25-local-llm: ${configDir.absolutePath}")
                    return configDir.absolutePath
                }
            }
            
            val lesson25Dir = File(searchDir, "lesson-25-local-llm")
            if (lesson25Dir.exists()) {
                configDir = File(lesson25Dir, "config")
                if (configDir.exists() && File(configDir, "server.yaml").exists()) {
                    logger.info("Found config in lesson-25-local-llm subdirectory: ${configDir.absolutePath}")
                    return configDir.absolutePath
                }
            }
        ...
        throw IllegalStateException(
            ...
            "Please ensure config/server.yaml exists in lesson-27-llm-vps directory."
        )
    }
}
