...
async function loadLLMStatus() {
    try {
        ...
        if (llmInfo && llmProvider && llmStatus) {
            ...
            if (data.localLLM) {
                ...
                if (localLLM.enabled) {
                    ...
                    const isVPS = localLLM.baseUrl.startsWith('https://');
                    const location = isVPS ? 'VPS' : 'локальная';
                    
                    // Применяем стили VPS к контейнеру
                    if (isVPS) {
                        llmInfo.classList.add('vps-active');
                        llmProvider.classList.add('vps-url');
                    } else {
                        llmInfo.classList.remove('vps-active');
                        llmProvider.classList.remove('vps-url');
                    }
                    
                    if (localLLM.available) {
                        if (isVPS) {
                            // Для VPS показываем URL сервера явно
                            llmProvider.textContent = `🌐 VPS: ${localLLM.baseUrl}`;
                            llmStatus.textContent = `✓ ${location} (${localLLM.model})`;
                            llmStatus.className = 'llm-status available vps';
                            llmStatus.title = `Подключено к VPS серверу: ${localLLM.baseUrl}\nМодель: ${localLLM.model}`;
                        } else {
                            // Для локальной LLM
                            llmProvider.textContent = data.provider || 'Локальная LLM';
                            llmStatus.textContent = `✓ ${location} (${localLLM.model})`;
                            llmStatus.className = 'llm-status available local';
                            llmStatus.title = `Подключено к локальной LLM: ${localLLM.baseUrl}\nМодель: ${localLLM.model}`;
                        }
                    } else {
                        if (isVPS) {
                            llmProvider.textContent = `🌐 VPS: ${localLLM.baseUrl}`;
                            llmStatus.textContent = `⚠ ${location} недоступна`;
                            llmStatus.className = 'llm-status unavailable vps';
                            llmStatus.title = `Не удалось подключиться к VPS серверу: ${localLLM.baseUrl}`;
                        } else {
                            llmProvider.textContent = data.provider || 'Локальная LLM';
                            llmStatus.textContent = `⚠ ${location} недоступна`;
                            llmStatus.className = 'llm-status unavailable local';
                            llmStatus.title = `Не удалось подключиться к локальной LLM: ${localLLM.baseUrl}`;
                        }
                    }
                } else {
                    llmInfo.classList.remove('vps-active');
                    llmProvider.classList.remove('vps-url');
                    llmProvider.textContent = data.provider || 'OpenRouter';
            ...
            // Показываем блок информации о LLM
