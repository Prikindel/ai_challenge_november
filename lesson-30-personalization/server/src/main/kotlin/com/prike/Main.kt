...
fun main(args: Array<String>) {
    ...
    logger.info("Starting lesson-30-personalization server...")
    ...
    embeddedServer(Netty, port = config.server.port, host = config.server.host) {
        ...
        routing {
            ...
            // Health check API
            get("/api/health") {
                call.respond(HttpStatusCode.OK, HealthResponse("ok", "lesson-30-personalization-server"))
            }
...
/**
 * Находит корень урока (lesson-30-personalization)
 */
private fun findLessonRoot(): File {
    ...
    // Ищем lesson-30-personalization вверх по дереву
    var searchDir: File? = currentDir
    while (searchDir != null) {
        if (searchDir.name == "lesson-30-personalization") {
            return searchDir
        }
        
        val lessonDir = File(searchDir, "lesson-30-personalization")
        if (lessonDir.exists()) {
            return lessonDir
        }
