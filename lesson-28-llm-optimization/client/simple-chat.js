...
async function sendMessage() {
    ...
    try {
        // Получаем сохранённые настройки из localStorage
        ...
        // Формируем запрос с использованием настроек
        const requestBody = {
            question: question
        };
        
        // Если есть сохранённые настройки, используем templateId
        if (savedSettings && savedSettings.templateId) {
            requestBody.templateId = savedSettings.templateId;
        } else {
            // Fallback на старый способ с systemPrompt
            requestBody.systemPrompt = 'Ты — полезный ассистент. Отвечай на вопросы пользователя дружелюбно и информативно.';
        }
