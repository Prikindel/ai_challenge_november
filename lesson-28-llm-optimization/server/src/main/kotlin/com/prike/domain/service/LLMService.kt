...
class LLMService(
    ...
    private val defaultTemplateId: String = "default"
) {
    ...
    private val localLLMClient: LocalLLMClient? = localLLMConfigValue?.let {
        if (it.enabled) {
            val authInfo = if (it.auth != null && it.auth.type != "none") {
                "auth=${it.auth.type}"
            } else {
                "no auth"
            }
            logger.info("Local LLM enabled: provider=${it.provider}, model=${it.model}, baseUrl=${it.baseUrl}, $authInfo")
            LocalLLMClient(
                baseUrl = it.baseUrl,
    ...
    suspend fun generateResponse(
        ...
        val finalParameters = parameters ?: (localLLMConfigValue?.parameters 
            ?: com.prike.domain.model.LLMParameters(temperature = defaultTemperature))
        
        logger.debug("Generating response with template '$templateIdToUse', parameters: temp=${finalParameters.temperature}, maxTokens=${finalParameters.maxTokens}, topK=${finalParameters.topK}, repeatPenalty=${finalParameters.repeatPenalty}")
        ...
        if (useLocalLLM && localLLMClient != null) {
            return try {
                ...
                val messages = mutableListOf<com.prike.data.dto.OllamaMessage>()
                
                // Добавляем system prompt отдельно, если есть
                systemPrompt?.let {
                    messages.add(com.prike.data.dto.OllamaMessage(
                        role = "system",
                        content = it
                    ))
                }
                ...
                return generateAnswerExternal(prompt, systemPrompt, finalParameters.temperature)
            } catch (e: Exception) {
    ...
    suspend fun generateAnswer(
        ...
        if (useLocalLLM && localLLMClient != null) {
            return try {
                ...
                // Используем параметры из конфигурации, но переопределяем temperature если указана
                val parameters = localLLMConfigValue?.parameters?.copy(temperature = temperature) 
                    ?: com.prike.domain.model.LLMParameters(temperature = temperature)
    ...
    fun getProviderInfo(): String {
        return if (useLocalLLM && localLLMConfigValue != null) {
            // Определяем, является ли это VPS (HTTPS URL)
            val isVPS = localLLMConfigValue.baseUrl.startsWith("https://")
            val location = if (isVPS) "vps" else "local"
            "local ($location: ${localLLMConfigValue.model})"
        } else {
