...
2. **Специализированные агенты:**
   ...
   - Используют `AIRepository` для получения данных от LLM (получают готовые строки)
...
3. **Репозитории в Data слое (`data/repository/`):**
   ...
   - Возвращают готовые данные (строки) агентам
   - Не содержат бизнес-логику, только получение данных от внешних источников
   - Пример: `AIRepository` - получает текстовые ответы от LLM API
...
│   └── AIRepository.kt          # Репозиторий для получения данных от LLM (используется агентами)
...
2. **Domain → Data:**
   - Агенты используют `AIRepository` для получения данных от LLM (получают готовые строки)
   - Агенты используют мапперы из `data/mapper/` для преобразования данных (строки → объекты)
