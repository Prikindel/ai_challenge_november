...
2. **Data (Слой данных)** - `data/`
   ...
   - Содержит:
     ...
     - `mapper/` - мапперы для преобразования данных (например, строк в объекты)
...
- Возвращает `CompletionResult` с JSON запросом и ответом для отладки

**Важно:** 
- Клиент инкапсулирует все детали API (ключи, URL, параметры), репозиторий не знает про эти детали
- `getCompletionWithHistory()` возвращает `CompletionResult`, содержащий `OpenAIResponse`, `requestJson` и `responseJson`
- JSON запросы и ответы логируются в формате OkHttp для удобной отладки
...
- Абстрактный класс для построения агентов
- Определяет общий контракт для работы с LLM через `AIRepository`
- Содержит базовые методы `getMessage()` и `getMessageWithHistory()`
- `getMessageWithHistory()` возвращает `AIRepository.MessageResult` с текстовым ответом и JSON запросом/ответом

**Специализированные агенты** (`domain/agent/`):
- Наследуют базовый агент (`BaseAgent`)
- Используют `AIRepository.getMessageWithHistory()` для получения `MessageResult` (текст + JSON)
- Используют мапперы из `data/mapper/` для преобразования строк в объекты
- Управляют историей сообщений через `MessageHistory` (опционально)
- Имеют свой системный промпт
- Содержат специфичную логику обработки результатов
- Могут хранить историю JSON запросов/ответов для отладки
...
├── MessageHistory.kt            # Управление историей сообщений (опционально)
├── TZAgent.kt                   # Специализированный агент (пример)
│   ├── Наследует BaseAgent
│   ├── Свой системный промпт
│   ├── Управление историей сообщений
│   └── Использование маппера для парсинга ответов
...
1. **Базовый агент (`BaseAgent`):**
   - Абстрактный класс с базовыми методами работы с LLM
   - Использует `AIRepository` для получения данных от LLM
   - `getMessage()` - простой запрос без истории
   - `getMessageWithHistory()` - запрос с историей, возвращает `MessageResult` с JSON

2. **Специализированные агенты:**
   - Наследуют `BaseAgent` (абстрактный класс)
   - Используют `AIRepository.getMessageWithHistory()` для получения `MessageResult` (текст + JSON)
   - Используют мапперы из `data/mapper/` для преобразования строк в объекты
   - Управляют историей сообщений через `MessageHistory` (опционально)
...
3. **Репозитории в Data слое (`data/repository/`):**
   - Работают с внешними API (например, `AIRepository` работает с `OpenAIClient`)
   - Возвращают готовые данные агентам
   - `getMessage()` - возвращает простой текстовый ответ
   - `getMessageWithHistory()` - возвращает `MessageResult` с текстом и JSON запросом/ответом
   - Не содержат бизнес-логику, только получение данных от внешних источников
   - Пример: `AIRepository` - получает текстовые ответы и JSON от LLM API

4. **Мапперы в Data слое (`data/mapper/`):**
   - Преобразуют данные из одного формата в другой (например, строки в объекты)
   - Выполняют парсинг, валидацию и трансформацию данных
   - Используются агентами для работы с данными от LLM
   - Не содержат бизнес-логику, только преобразование данных
   - Пример: `TZMapper` - преобразует строковые ответы от LLM в объекты `TechnicalSpec`
...
Контроллер
  ↓
Специализированный агент (TZAgent)
  ↓ (использует)
AIRepository.getMessageWithHistory() → MessageResult (текст + JSON)
  ↓ (использует)
OpenAIClient.getCompletionWithHistory() → CompletionResult (ответ + JSON)
  ↓ (отправляет запрос)
LLM API
  ↓ (возвращает ответ)
OpenAIClient → AIRepository → TZAgent (использует маппер для парсинга)
  ↓
...
presentation/
├── controller/
│   └── ChatController.kt         # Вызывает агентов напрямую
...
data/
├── client/
│   └── OpenAIClient.kt           # HTTP клиент для LLM API (возвращает CompletionResult)
├── dto/                          # DTO для LLM API (не используются в Presentation)
├── repository/                   # Репозитории для работы с внешними API
│   └── AIRepository.kt           # Репозиторий для получения данных от LLM (возвращает MessageResult)
└── mapper/                       # Мапперы для преобразования данных
    └── TZMapper.kt               # Маппер для преобразования строк в объекты (пример)
```
...
2. **Domain → Data:**
   - Агенты используют `AIRepository` для получения данных от LLM
   - `AIRepository.getMessageWithHistory()` возвращает `MessageResult` (текст + JSON запрос/ответ)
   - Агенты используют мапперы из `data/mapper/` для преобразования строк в объекты
   - `AIRepository` использует `OpenAIClient` из `data/client/` для запросов к LLM
   - `OpenAIClient.getCompletionWithHistory()` возвращает `CompletionResult` (ответ + JSON)
   - Репозитории работают с DTO из `data/dto/` и предоставляют данные агентам
   - Агенты используют исключения из `domain/exception/`

3. **Domain → Domain:**
   - Специализированные агенты наследуют базовый агент (`BaseAgent`)
...
5. **Запрещено:**
   ...
   - Репозитории в `data/repository/` работают с внешними API, возвращают готовые данные
   - Мапперы в `data/mapper/` преобразуют данные (строки → объекты), не содержат бизнес-логику
...
3. **Добавить логику:**
   - Создать контроллеры в `presentation/controller/` (вызывают агентов)
   - Создать клиентские DTO в `presentation/dto/` (только примитивы)
   - Создать специализированные агенты в `domain/agent/` (наследуют `BaseAgent`)
   - При необходимости создать мапперы в `data/mapper/` для преобразования данных
   - При необходимости создать `MessageHistory` для управления историей сообщений
