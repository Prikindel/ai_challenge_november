...
class KnowledgeBaseSearchService(
    ...
    /**
     * Выполняет поиск только в документации проекта (project/docs/ и project/README.md)
     * 
     * @param query поисковый запрос
     * @param limit максимальное количество результатов
     * @param minSimilarity минимальное сходство (по умолчанию 0.0)
     * @return список результатов поиска только из документации проекта
     */
    suspend fun searchProjectDocs(
        ...
        logger.debug("Searching project docs for: $query (limit: $limit, minSimilarity: $minSimilarity)")
        ...
        // 4. Получаем документы для фильтрации по пути
        val allDocuments = knowledgeBaseRepository.getAllDocuments()
        val projectDocs = allDocuments.filter { doc ->
            doc.filePath.startsWith("project/docs/") || doc.filePath == "project/README.md"
        }
        val projectDocIds = projectDocs.map { it.id }.toSet()
        
        if (projectDocIds.isEmpty()) {
            logger.warn("No project documentation found in knowledge base")
            return emptyList()
        }
        
        logger.debug("Filtering ${allChunks.size} chunks to ${projectDocIds.size} project documents")
        
        // 5. Фильтруем чанки только из документации проекта
        val projectChunks = allChunks.filter { chunk ->
            projectDocIds.contains(chunk.documentId)
        }
        
        if (projectChunks.isEmpty()) {
            logger.warn("No chunks found in project documentation")
            return emptyList()
        }
        
        // 6. Вычисление сходства для каждого чанка
        val results = projectChunks.map { chunk ->
            val similarity = similarityCalculator.calculateSimilarity(
                normalizedQuery,
                chunk.embedding
            )
        ...
        // 7. Фильтрация по минимальному сходству, сортировка и возврат топ-N
        val topResults = results
            .filter { it.similarity >= minSimilarity }
            .sortedByDescending { it.similarity }
            .take(limit)
        
        // 8. Обогащаем результаты информацией о документах
        val documentsMap = projectDocs.associateBy { it.id }
