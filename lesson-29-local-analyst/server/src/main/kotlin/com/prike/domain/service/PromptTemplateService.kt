...
class PromptTemplateService(
    private val repository: PromptTemplateRepository = PromptTemplateRepository()
) {
    ...
    fun applyTemplate(
        ...
        if (context != null) {
            result = result.replace("{data}", context)
        } else {
            // Удаляем плейсхолдер данных, если он есть, но данные не предоставлены
